...
{ $EXECDIR/ndzip/build/compress -e cuda -i $DATADIR/h3d_temp -o $DATADIR/h3d_temp.ndzip2 -t float -n 100 500 500 ; } 2>>$RESDIR/res_ndzip_gpu.txt
{ $EXECDIR/ndzip/build/compress -e cuda -i $DATADIR/hdr_night -o $DATADIR/hdr_night.ndzip2 -t float -n 5687448 ; } 2>>$RESDIR/res_ndzip_gpu.txt
...
{ $EXECDIR/ndzip/build/compress -d -e cuda -o $DATADIR/h3d_temp.ndzout2 -i $DATADIR/h3d_temp.ndzip2 -t float -n 100 500 500 ; } 2>> $RESDIR/res_ndzip_gpu_decomp.txt
{ $EXECDIR/ndzip/build/compress -d -e cuda -o $DATADIR/hdr_night.ndzout2 -i $DATADIR/hdr_night.ndzip2 -t float -n 5687448 ; } 2>> $RESDIR/res_ndzip_gpu_decomp.txt
